...
def test_stratified_srswor_select_same_size():
    size = 2
    str_srswor_sample, str_srswor_number_hits, str_srswor_probs = str_srswor_design.select(
...
def test_ppswr_sys_select_shuffle():
    pps_sample_shuffled, pps_hits_shuffled, pps_probs_shuffled = pps_sys_design_wr.select(
...
def test__calculate_samp_size_dict():
    ...
    samp_size = some_design._calculate_samp_size(
        strat=True, pop_size={"one": 100, "two": 200}, samp_rate={"one": 0.1, "two": 0.20}
    )
