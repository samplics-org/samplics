...
The module *datasets* allows the user to load the datasets used in this tutorial. Note that the datasets are only used to illustrate the syntax and APIs of *samplics*. The datasets should not be used for any statistical analysis. **Many of the datasets in this tutorial are subsets of actual samples but DO NOT represent these samples**. The datasets were subseted from existing samples to reduce the size of the files.  Each dataset is represented by a class with the members: *name*, *description*, *nrows*, *ncols*, *design*, *source*, and, *data*. The list of datasets are the following:
