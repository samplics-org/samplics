...
class CrossTabulation:
    ...
    def _to_polars(self, row_varname, column_varname):
        df = (
            pl.DataFrame(
                self._flatten_dict(self.point_est, row_varname, column_varname, stat="point_est")
            )
            .join(
                pl.DataFrame(
                    self._flatten_dict(self.stderror, row_varname, column_varname, stat="stderror")
                ),
            ...
            .join(
                pl.DataFrame(
                    self._flatten_dict(self.lower_ci, row_varname, column_varname, stat="lower_ci")
                ),
                on=[row_varname, column_varname],
                how="left",
            )
            .join(
                pl.DataFrame(
                    self._flatten_dict(self.upper_ci, row_varname, column_varname, stat="upper_ci")
                ),
                on=[row_varname, column_varname],
                how="left",
            )
